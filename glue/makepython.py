...
def makePythonShims(shimfiles):
    ...
    with open("../src/super/pythonpreamble.py","w") as fp:

        #standard header
        print(file=fp)
        print(file=fp)
        print(f"# This file was autogenerated by {os.path.basename(__file__)}. Do not edit.",file=fp)
        print(file=fp)
        print(file=fp)
        with open("shims/print_.py") as fp2:
            data = fp2.read()
            #we can't put this into print_.py because
            #importlib will try to run it when we execute
            #our build scripts
            print("import browser  #type: ignore",file=fp)
            print(data,file=fp)


    pyfile = "../src/super/quin.py"
...
def generatePythonShim(name,fp,func):
    ...
    if src2 == "pass":
        #no function body provided, so we synthesize
