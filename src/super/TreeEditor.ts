...
//the icons we know about. The images for these are kept
//in the icons/ folder
const knownIcons = ["box","cylinder","difference","intersection","sphere","union"];
...
export class TreeEditor{
    ...
    constructor(treeContainer: HTMLElement, propertyContainer: HTMLElement){
        ...
        let icons:any={};
        knownIcons.forEach( (name:string) => {
            icons[name] = {};
            icons[name].icon = `./icons/${name}.svg`;
        });
        ...
        let tree = $(div).jstree({
            ...
            plugins: [
                ...
                //if wholerow is active, we can't add additional widgets
                //like checkboxes to the li item
        ...
        $(tree).on("select_node.jstree", (e: Event,data: any)=>{
            ...
            let g: GeometricObject[] = [];
            selected.forEach( (s: any) => {
                let t: TreeNode = s.original;
                g.push(t.geo);
            });
        ...
        ObjectDepot.addAddObjectListener( (obj: GeometricObject) => {
            ...
            //FIXME: Investigate: Can we omit the 'true'?

            var treeNode = new TreeNode(obj.name, obj.iconName,
                obj);
            
            obj.treeNode = treeNode;
            treeNode.geo = obj;

            var newnodeid = $(div).jstree().create_node(
                "#",       //parent
                treeNode,     //data for node
                "last",     //position: first, last, or index
                (n: any) => {
                    console.log("Node was created",n);
                    let jn = document.getElementById(n.id);
                    let anchors = jn.getElementsByTagName("a");
                    let cbox = document.createElement("input");
                    cbox.type="checkbox";
                    jn.appendChild(cbox);
                    console.log(cbox);
                    // cbox.addEventListener("click", (ev: MouseEvent)=>{
            ...
            treeNode.jstreeID = newnodeid;

            console.log("new node id:",newnodeid);
            let jn = document.getElementById(newnodeid);
            console.log("node?",jn);
